...
parser.add_argument('--data_dir', type=str, default='dataset/definition/lca')
...
parser.add_argument('--mi_loss', type=float, default=100.0, help='')
...
file_logger = helper.FileLogger(model_save_dir + '/' + opt['log'], header="# epoch\ttrain_loss\tsent_loss\tdep_path_loss\tmi_loss\tdev_loss\tdev_score\tbest_dev_score")
...
format_str = '{}: step {}/{} (epoch {}/{}), loss = {:.6f}, sent_loss = {:.6f}, dep_path_loss = {:.6f}, mi_loss = {:.6f} ({:.3f} sec/batch), lr: {:.6f}'
...
for epoch in range(1, opt['num_epoch']+1):
    ...
    for i, batch in enumerate(train_batch):
        ...
        global_step += 1
        loss, sent_loss, dep_path_loss, mi_loss = trainer.update(batch)
        train_loss += loss
        train_sent_loss += sent_loss
        train_mi_loss += mi_loss
        train_dep_path_loss += dep_path_loss
        if global_step % opt['log_step'] == 0:
            duration = time.time() - start_time
            print(format_str.format(datetime.now(), global_step, max_steps, epoch,\
                    opt['num_epoch'], loss, sent_loss, dep_path_loss, train_mi_loss, duration, current_lr))
    ...
    train_mi_loss = train_mi_loss / train_batch.num_examples * opt['batch_size'] # avg loss per batch
    dev_loss = dev_loss / dev_batch.num_examples * opt['batch_size']

    dev_p, dev_r, dev_f1 = scorer.score(dev_batch.gold(), predictions, method='macro')
    print("epoch {}: train_loss = {:.6f}, train_sent_loss = {:.6f}, train_dep_path_loss = {:.6f}, train_mi_loss = {:.6f}, dev_loss = {:.6f}, dev_f1 = {:.4f}".format(epoch,\
        train_loss, train_sent_loss, train_dep_path_loss, train_mi_loss, dev_loss, dev_f1))
    dev_score = dev_f1
    file_logger.log("{}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.4f}\t{:.4f}".format(epoch, train_loss, train_sent_loss, train_dep_path_loss, train_mi_loss, dev_loss, dev_score, max([dev_score] + dev_score_history)))
